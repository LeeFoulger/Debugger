...
void c_debugger::run_debugger(bool print_debug_strings)
{
	...
	while (m_process_is_attached)
	{
		...
		{
			m_continue_status = exception_handler();
			break;
		}
		case OUTPUT_DEBUG_STRING_EVENT:
		{
			m_continue_status = output_debug_string_handler();
			break;
		}
...
DWORD c_debugger::exception_handler()
{
	static CONTEXT context = { 0 };

	static bool first_run = true;

	static HMODULE modules[1] = { 0 };
	static MODULEINFO module_info = { 0 };

	static SIZE_T bytes_written = 0;
	static SIZE_T bytes_read = 0;

	static SIZE_T last_instruction_pointer = 0;

	static BYTE instructions[READ_PAGE_SIZE] = { 0 };

	static DWORD breakpoints_set = 0;

	static WCHAR module_name[MAX_PATH] = { 0 };

	memset(modules, 0, sizeof(modules));
	memset(instructions, 0, sizeof(instructions));
	memset(module_name, 0, sizeof(module_name));

	if (m_debug_event.u.Exception.ExceptionRecord.ExceptionCode == EXCEPTION_BREAKPOINT)
	{
		if (first_run)
		{
			first_run = false;
			...
			for (SIZE_T i = 0; i < m_breakpoints.count; i++)
			{
				...
				for (SIZE_T c = 0; c < bytes_read; c++)
				{
					...
					if (breakpoints_set < m_breakpoints.get_max_count())
					{
						...
						FlushInstructionCache(m_process.get_process_handle(), (LPVOID)offset, sizeof(k_break_instruction));
		...
		else
		{
			...
			if (m_thread_handle != NULL)
			{
				...
				if (instruction)
				{
					...
					FlushInstructionCache(m_process.get_process_handle(), (LPVOID)context.Xip, sizeof(k_break_instruction));
				}
	...
	else if (m_debug_event.u.Exception.ExceptionRecord.ExceptionCode == EXCEPTION_SINGLE_STEP)
	{
		m_thread_handle = OpenThread(THREAD_ALL_ACCESS, true, m_debug_event.dwThreadId);
		if (m_thread_handle != NULL)
		{
			...
			if (last_instruction_pointer)
			{
				...
				FlushInstructionCache(m_process.get_process_handle(), (LPVOID)last_instruction_pointer, sizeof(k_break_instruction));
...
DWORD c_debugger::output_debug_string_handler()
{
